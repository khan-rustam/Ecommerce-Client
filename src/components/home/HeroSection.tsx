...
const HeroSection: React.FC<HeroProps> = ({ title, subtitle, buttonText, buttonLink, backgroundImage, titleFontSize, titleFontWeight, subtitleFontSize, subtitleFontWeight }) => {
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      ...
      try {
        ...
        bannersArray = bannersArray.filter((b: Banner) => b && typeof b === "object" && typeof b.show !== "undefined");
  ...
  return (
    <div
      className="relative h-[45vh] md:h-[55vh] w-full overflow-hidden py-2 flex items-center justify-center font-sans"
      style={{fontFamily:'Inter, Poppins, sans-serif'}}
    >
      {/* Banner image/slider as before */}
      {loading ? (
        <div className="h-64 flex items-center justify-center text-primary animate-pulse font-semibold text-xl">
          Loading banners...
        </div>
      ) : error ? (
        <div className="h-64 flex items-center justify-center text-red-500 font-semibold">
          {error}
        </div>
      ) : (
        <Slider
                  ...
                  <img
                    ...
                    onError={(e) => {
                      (e.target as HTMLImageElement).src =
                        "https://placehold.co/1200x400?text=Error";
                    }}
                  />
                  {banner.name && (
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/40 to-black/10 z-10 pointer-events-none"></div>
      {/* Centered text and CTA */}
      <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center px-4">
        <h1 className="text-4xl md:text-6xl font-extrabold text-white drop-shadow-lg mb-4 animate-fade-in-up" style={{letterSpacing:'-0.03em'}}>
          {title}
        </h1>
        <p className="text-lg md:text-2xl text-white/90 font-medium mb-8 animate-fade-in-up" style={{animationDelay:'0.2s'}}>
          {subtitle}
        </p>
        <a
          href={buttonLink}
          className="inline-block px-8 py-3 rounded-full bg-gradient-to-r from-accent to-primary text-white font-bold text-lg shadow-lg hover:from-primary hover:to-accent transition-all duration-300 animate-fade-in-up"
