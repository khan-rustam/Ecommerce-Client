...
const PaymentsPage = () => {
  ...
  return (
    <AccountLayout>
      <h2 className="text-2xl font-bold mb-6" style={{ color: colors.primary }}>Payment Methods</h2>
      <button
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {payments.map(card => (
            <div key={card._id} className="border rounded-xl p-5 flex justify-between items-center bg-white shadow-md hover:shadow-lg transition-shadow duration-200 group" style={{ borderColor: colors.secondary }}>
              <div>
                <div className="font-semibold text-lg flex items-center gap-2">
                  <CreditCard style={{ color: colors.primary }} size={20} />
                  <span style={{ color: colors.text }}>{card.type} ending in {card.last4}</span>
                </div>
                <div className="text-sm mt-1" style={{ color: colors.text }}>Expires {card.expiry}</div>
              </div>
              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                <button className="hover:underline font-medium" style={{ color: colors.primary }} onClick={() => handleEdit(card)}>Edit</button>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md relative z-10 animate-fade-in">
            ...
            <h3 className="text-xl font-bold mb-4 text-center" style={{ color: colors.primary }}>{editId ? 'Edit Card' : 'Add Card'}</h3>
      ...
      {deleteId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-sm relative z-10 animate-fade-in flex flex-col items-center">
            <h3 className="text-xl font-bold mb-4 text-center" style={{ color: colors.primary }}>Remove Card</h3>
            <p className="mb-6 text-center" style={{ color: colors.text }}>Are you sure you want to remove this card? This action cannot be undone.</p>
