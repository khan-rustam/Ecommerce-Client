...
const AdminCallbackRequest: React.FC = () => {
  ...
  return (
    <div className="p-6 md:p-10 max-w-4xl mx-auto" style={{ background: 'var(--brand-bg)', color: 'var(--brand-text)' }}>
      <h1 className="text-3xl font-bold mb-8 text-center" style={{ color: 'var(--brand-primary)' }}>Callback Requests</h1>
      {loading ? (
          ...
          {requests.map(req => (
            <div key={req._id} className="bg-white rounded-xl shadow border border-slate-100 p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4 transition hover:shadow-lg">
              <div className="flex items-center gap-4">
                <div className="rounded-full bg-[var(--brand-primary,#f3f4f6)] w-14 h-14 flex items-center justify-center text-2xl font-bold text-white" style={{ background: 'var(--brand-primary)' }}>
                  <Phone size={28} />
                </div>
                <div>
                  <div className="font-semibold text-lg" style={{ color: 'var(--brand-primary)' }}>{req.name}</div>
              ...
              <div className="flex flex-col gap-2 md:items-end md:justify-center">
                ...
                <a href={`tel:${req.phone}`} className="flex items-center gap-2 px-4 py-2 rounded bg-green-600 text-white hover:bg-green-700 transition shadow text-center" style={{ background: 'var(--brand-primary)' }}>
