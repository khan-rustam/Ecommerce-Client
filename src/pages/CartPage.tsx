...
import amexImg from '../assets/Payments/americanexpress.png';
...
const CartPage = () => {
  ...
  // Helper to get product id (supports both id and _id)
  const getProductId = (product: { id?: string | number; _id?: string | number }) => product.id || product._id;
  // Helper to get product image (supports string or object with url)
  const getProductImage = (product: any) => {
    if (!product.images || product.images.length === 0) return "https://via.placeholder.com/80x80?text=No+Image";
          ...
          <div className="bg-white rounded-lg shadow-sm overflow-hidden">
            ...
            <div className="divide-y divide-gray-200">
              {cartItems.map(({ product, quantity }) => (
                <div key={getProductId(product)} className="grid grid-cols-1 md:grid-cols-5 gap-4 p-4 md:p-6 items-center">
                  {/* Product */}
                  <div className="col-span-1 md:col-span-2 flex items-center">
                    <Link to={`/product/${getProductId(product)}`} className="shrink-0 w-20 h-20 bg-gray-100 rounded overflow-hidden mr-4">
                      <img
                    ...
                    <div>
                      <Link to={`/product/${getProductId(product)}`} className="font-medium hover:text-orange-500">
                        {product.name}
                      </Link>
                      <button
